...
const STATUS_COLOR: Record<string,string> = {
  new:'#3b82f6', awaiting_prepayment:'#f59e0b', prepayment_review:'#8b5cf6',
...
export default function AdminDashboard() {
  ...
  const statCards = stats ? [
    { label:'Всего броней', value:stats.total_bookings, icon:'📋', color:'#3b82f6' },
    ...
    <div className="p-5 max-w-7xl mx-auto">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/admin/bookings" className="text-sm font-medium transition-colors" style={{color:'#60a5fa'}}>Все →</Link>
        </div>
          ...
          {recent.map(b=>(
            <Link key={b.id} href={`/admin/bookings/${b.id}`} className="card p-4 hover:border-blue-500/30 transition-all block group">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <span className="text-xs font-black font-mono" style={{color:'#60a5fa'}}>{b.booking_number}</span>
